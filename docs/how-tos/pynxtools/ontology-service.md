...
The FastAPI app is defined in [`nomad_ontology_service.apis.app`](https://github.com/FAIRmat-NFDI/nomad-ontology-service/blob/main/src/nomad_ontology_service/apis/app.py), in the `nomad-ontology-service` plugin (not in `pynxtools`). On the `pynxtools` side, `ensure_ontology_initialization` (in [`pynxtools.nomad`](https://github.com/FAIRmat-NFDI/pynxtools/blob/master/src/pynxtools/nomad/__init__.py)) generates the ontology file that service reads:
...
3. **Storing results**: The retrieved superclasses are stored in `results.eln.methods` in the NOMAD archive.
    ...
    ![Ontology results in NOMAD](../../assets/superclasses_results_1.png){ width="800" }
    ![Ontology results in NOMAD](../../assets/superclasses_results_2.png){ width="800" }
