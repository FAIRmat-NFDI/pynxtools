...
- **`NXdata`**: The [**`NXdata`**](https://manual.nexusformat.org/classes/base_classes/NXdata.html#nxdata) group plays a special role in NeXus as it is used to define [the plottable data](https://manual.nexusformat.org/examples/python/plotting/index.html). Identification of the signal and axes follows the three NeXus Data Plotting Standard conventions (v3/NIAC2014, v2/~2004, v1/oldest) implemented in `pynxtools.nexus.nxdata`. When a `@signal` attribute is present, the validator checks that the named dataset exists, that all declared axes exist and are dimensionally consistent with the signal, and that any `AXISNAME_indices` attributes reference valid dimensions.
