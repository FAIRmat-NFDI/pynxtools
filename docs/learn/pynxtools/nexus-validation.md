...
- **`NXdata`**: The [**`NXdata`**](https://manual.nexusformat.org/classes/base_classes/NXdata.html#nxdata) group plays a special role in NeXus as it not only stores [the plottable data](https://manual.nexusformat.org/examples/python/plotting/index.html), but also holds annotations that can be used to guide software tools on how to plot and slice these data. Identification of the signal and axes follows the three NeXus Data Plotting Standard conventions (v3/NIAC2014, v2/~2004, v1/oldest) implemented in `pynxtools.nexus.nxdata`. When a `@signal` attribute is present, the validator checks that the named dataset exists, that all declared axes exist and are dimensionally consistent with the signal, and that any `AXISNAME_indices` attributes reference valid dimensions.- **Links**: When writing NeXus files in HDF5, any group or field can be replaced by an [HDF5 link](https://manual.nexusformat.org/design.html#links) to another group/field. In the validation, these links are resolved and the validity of the resolved object against the NeXus definition is checked. In addition, the presence of the `@target` attribute (which is supposed to be set when using links) is checked.
