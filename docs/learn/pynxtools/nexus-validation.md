...
Both `pynx validate` and `pynx read` are backed by the same `NexusFileHandler` traversal; they differ only in the `NexusVisitor` they use (`ValidationVisitor` vs. `Annotator`).
