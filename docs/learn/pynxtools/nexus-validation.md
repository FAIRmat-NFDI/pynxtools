...
- **`NXdata`**: The [**`NXdata`**](https://manual.nexusformat.org/classes/base_classes/NXdata.html#nxdata) group plays a special role in NeXus as it is used to define [the plottable data](https://manual.nexusformat.org/examples/python/plotting/index.html). Identification of the signal and axes follows the three NeXus Data Plotting Standard conventions (v3/NIAC2014, v2/~2004, v1/oldest) implemented in `pynxtools.nexus.nxdata`. The following structural constraints are checked and reported as **WARNING**-level messages by both `pynx validate` and `pynx read`:

    | # | Rule | What is checked |
    |---|------|-----------------|
    | 1 | **`@axes` rank** | The number of entries in the `@axes` attribute must equal the number of dimensions of the signal dataset. |
    | 2 | **`AXISNAME_indices` count** | If an `AXISNAME_indices` attribute is present, the number of entries it contains must equal the number of dimensions of the corresponding axis dataset. |
    | 3 | **Missing signal** | The dataset named by `@signal` must exist in the group. |
    | 4 | **Missing axes** | Every axis name listed in `@axes` or referenced by `AXISNAME_indices` must exist as a dataset in the group. |
    | 5 | **Indices subset** | For each axis, the dimension positions implied by the `@axes` ordering must be a subset of (or equal to) those declared in `AXISNAME_indices`. |
    | 6 | **Shape correspondence** | For each dimension `i` listed in `AXISNAME_indices`, `AXISNAME.shape[i]` must equal `signal.shape[AXISNAME_indices[i]]`. A bin-edge tolerance of `+1` is allowed (i.e., the axis may have one extra point to represent bin edges). |
    | 7 | **Auxiliary signal shape** | Each dataset listed in `@auxiliary_signals` must have the same shape as the primary signal. |
    | 8 | **`FIELDNAME_errors` shape** | If a `{name}_errors` dataset is present (where `{name}` is the signal, an axis, or an auxiliary signal name), its shape must exactly match the shape of the corresponding `{name}` dataset. |
    | 9 | **No signal convention** | If an `NXdata` group contains datasets but no signal can be identified by any of the three plotting conventions (i.e., no `@signal` attribute and no v1/v2 fallback), a warning is raised. |

    All of these checks are implemented in the pure function `check_nxdata` in `pynxtools.nexus.nxdata`, which returns a list of `NXdataViolation` objects that both the validator and the annotator consume.
