...
#
"""v2 port of test_parsing_opt.py: NexusParserV2 with the same storage-layout/dtype matrix.

Group names ("event1"/"image1") are unchanged from v1 — NXDL ``nameType="partial"``
treats uppercase letter runs in the given name (e.g. the "ID" in "eventID") as
wildcards, so "event1" and "eventID1" are equally valid matches for the
``eventID`` group. ``_SectionIndex.find_subsection`` previously approximated this
with a naive ``.replace("GROUPNAME", "").replace("NAME", "")`` + ``startswith``
