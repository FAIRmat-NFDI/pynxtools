...
def xml_handle_links(obj, keyword, value):
...
def recursive_build(obj, dct):
    ...
    for keyword, value in iter(dct.items()):
        ...
        #handle special keywords (symbols), assumed that you do not encounter further symbols nested inside
            ...
            xml_handle_links(obj, keyword, value)
        #base/cand classes, attributes, and special keywords (symbols) handled, so only members of nexus classes remain to handle, which will populate xml fields
        elif kName != '': 
            typ = 'NX_CHAR'
            ...
            fld.set('name', kName)
            fld.set('type', typ)
            if value != None: #a field may have subordinated attributes
                if isinstance(value,dict):
                    for kkeyword, vvalue in iter(value.items()):
