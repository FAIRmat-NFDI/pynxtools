...
class ApmDefaultPlotGenerator:  # pylint: disable=R0903
    """Wrapper for generating default plots."""

    def __init__(self):
        self.meta: Dict[str, NxObject] = {}

    def create_default_plot_reconstruction(self, template: dict) -> dict:
    ...
    def create_default_plot_mass_spectrum(self, template: dict) -> dict:
        """Compute on-the-fly, add, and give path to discretized reconstruction."""
    ...
    def report(self, template: dict) -> dict:
        ...
        if has_valid_xyz is True:
            self.create_default_plot_reconstruction(template)
        ...
        if has_valid_m_z is True:
            self.create_default_plot_mass_spectrum(template)
