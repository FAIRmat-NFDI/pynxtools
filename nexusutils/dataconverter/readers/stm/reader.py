...
def get_nested_dict_from_nested_key_dict(data_dict, dict_to_map_path=None):
    ...
    for d_key, d_val in data_dict.items():
        #any_sep_char = [x in d_key for x in separared_char]
...
def signal_obj_to_template_dict(signal):
    ...
    for nx_full_path, val in path_to_val_items:
        ...
        template_dict.update(construct_nexus_data_path(nx_full_path,
...
class STMReader(BaseReader):
   ...
   # NXroot is a general purpose definition one can review data with this definition
    ...
    def read(self,
        ...
        """
        """
        file_name = ('/home/rubel/Nomad-FAIRmat/NomadGH/GH_Clone/nomad-parser-nexus/nexusutils/'
                     'dataconverter/readers/stm/SPM_data_folder/'
                     'TiSe2_2303a_annealing_300C_5min_evaporate_Pyrene_1_0070.sxm')

        clean_template = Template()
        filled_template = pass_metadata_and_signal_into_template(clean_template, file_name)
