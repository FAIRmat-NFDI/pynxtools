...
used file formats from technology partners of electron microscopy software tools
into a NeXus/HDF5 file which complies with a specific version of the NXem application
definition.
...
3. Drag-and-drop your files in specific file formats, i.e. bcf, emd, dm3.
3. Edit the electronic lab notebook (ELN) schema inside the NOMAD OASIS and click the
   ...
   file(s) from technology partners and the eln_data.yaml file. Afterwards, the file will be
...
4. If successful, a NeXus (nxs) file will appear in your upload. You can explore
   ...
   of the material region-of-interest (ROI) characterized, and if present in composition maps.
...
from an arbitrary such microscopy tool and its often specific formatting. Therefore,
the em_spctrscpy reader takes such additional metadata from the eln_data.yaml file.
Specifically, this file can be created with the ELN schema creation and customization
functionalities are offered through NOMAD OASIS or via any other ELN
or solution with which users can edit yaml files.
...
formats and use cases, from results of interactions with different GUIs, scripts
...
Practically, the process needs to be driven by examples because many I/O routines for especially
file formats from technology partners inside open-source software was designed by reverse engineering.
Hyperspy is a good example of these developments and efforts.
...
Thus, supporting the development and working together on I/O functions for files
from technology partners and discussing open issues is in our opinion the preferred approach.
...
some of the file formats from technology partners. We should make clear that we acknowledge
the significant momentum and effort microscopy manufacturers as technology partners invest
into the storing of metadata in their (commercial) software solutions.
We would like to see though that more of these efforts become documented in the
literature and the public domain.
...
especially for some file formats is neither exhaustive nor documented in detail enough.
