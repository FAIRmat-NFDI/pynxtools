...
- Data Converter: The __DataConverter__ as described above is also available in NOMAD. Hence, NOMAD users can directly convert their experimental data to NeXus using the NOMAD GUI. Aside from the capabilities already described, the internal __DataConverter__ class also handles NOMAD's electronic lab notebooks (ELNs) and converts these such that the manually inputted data can be converted to NeXus as well.
