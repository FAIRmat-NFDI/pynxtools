...
def does_path_exist(path, h5py_obj) -> bool:
    ...
    try:
        h5py_obj[
            helpers.convert_data_dict_path_to_hdf5_path(path)
...
class Writer:
    ...
    def __init__(
        self, data: dict = None, nxdl_path: str = None, output_path: str = None
    ):
    ...
    def write(self):
        ...
        for path, value in self.data.items():
            try:
                    ...
                    if isinstance(data, dict):
                        if "compress" in data.keys():
                            dataset = handle_dicts_entries(
                                data, grp, entry_name, self.output_path
                            )
                        else:
                ...
                raise Exception(
                    f"Unkown error occured writing the path: {path} "
