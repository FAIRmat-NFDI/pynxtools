        ...
        def indivisual_DATA_field():
            ...
            for path in dt_path_list:
                if path in data_dict:
                    ...
                    scan_dt_arr = transform_to_intended_dt(data_dict[path])
    ...
    def get_dimension_info(self, config_dict, data_dict):
        ...
        for sep in [";"]:
            if sep in scanfield:
                ...
                x_start = transform_to_intended_dt(scanfield_parts[0])
                x_len = transform_to_intended_dt(scanfield_parts[2])
                x_cor = [x_start, x_start + x_len, unit_info]
                y_start = transform_to_intended_dt(scanfield_parts[1])
                y_len = transform_to_intended_dt(scanfield_parts[3])
                ...
                if isinstance(c_val, list):
                    for search_key in c_val:
                        if search_key in data_dict:
                            template[c_key] = transform_to_intended_dt(data_dict[search_key])
