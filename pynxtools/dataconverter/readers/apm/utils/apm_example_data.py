...
from pynxtools.dataconverter.readers.apm.utils.apm_ranging_io import add_unknown_iontype
...
class ApmCreateExampleData:
    ...
    def place_atoms_from_periodic_table(self):
        ...
        self.charge_state = np.asarray(
            np.random.uniform(low=1, high=MAX_CHARGE, size=(self.n_components,)),
            np.uint32,
        )
    ...
    def composition_to_ranging_definitions(self, template: dict) -> dict:
        ...
        trg = (
            "/ENTRY[entry"
            + str(self.entry_id)
            + "]/atom_probe/ranging/peak_identification/"
        )
        template[trg + "program"] = "synthetic"
        ...
        for tpl in self.nrm_composition:
            ...
            template[path + "isotope_vector"] = np.reshape(
            ...
            template[path + "mass_to_charge_range"] = np.reshape(
    ...
    def emulate_entry(self, template: dict) -> dict:
        ...
        template[trg + "run_number"] = experiment_identifier.split("-")[1]
        template[trg + "operation_mode"] = str(
    ...
    def emulate_specimen(self, template: dict) -> dict:
        ...
        specimen_name = "Mocked atom probe specimen " + str(
    ...
    def emulate_control_software(self, template: dict) -> dict:
        ...
        template[trg + "program/@version"] = (
            "3." + str(np.random.choice(9, 1)[0]) + "." + str(np.random.choice(9, 1)[0])
        )
        return template
    ...
    def emulate_instrument_header(self, template: dict) -> dict:
        ...
        template[trg + "instrument_name"] = "test instrument " + str(
            np.random.choice(100, 1)[0]
        )
    ...
    def emulate_fabrication(self, template: dict) -> dict:
        ...
        trg = (
            "/ENTRY[entry"
            + str(self.entry_id)
            + "]/atom_probe/FABRICATION[fabrication]/"
        )
        template[trg + "vendor"] = str(
            np.random.choice(["AMETEK/Cameca", "customized"], 1)[0]
        )
        template[trg + "model"] = str(
            np.random.choice(
                [
                    "LEAP3000",
                    "LEAP4000",
    ...
    def emulate_reflectron(self, template: dict) -> dict:
        ...
        trg = (
            "/ENTRY[entry" + str(self.entry_id) + "]/atom_probe/REFLECTRON[reflectron]/"
        )
    ...
    def emulate_local_electrode(self, template: dict) -> dict:
        ...
        template[trg + "name"] = "electrode " + str(np.random.choice(1000, 1)[0])
        return template
    ...
    def emulate_specimen_monitoring(self, template: dict) -> dict:
        ...
        template[trg + "shank_angle"] = np.float64(0.0)  # = np.random.choice(10, 1)[0]
    ...
    def emulate_pulser(self, template: dict) -> dict:
        ...
        template[trg + "pulse_fraction"] = np.float64(
    ...
    def emulate_reconstruction(self, template: dict) -> dict:
        ...
        template[trg + "protocol_name"] = str(
