...
def add_standardize_molecular_ions(
    ...
    for ion in ion_lst:
        ...
        template[f"{path}mass_to_charge_range"] = np.array(
            ion.ranges.typed_value, np.float32
        )
        ...
        template[f"{path}isotope_matrix"] = {
            "compress": np.array(ion.charge_state_model["isotope_matrix"], np.uint16),
            "strength": 1,
        }
        ...
        template[f"{path}mass_vector"] = {
            "compress": np.array(ion.charge_state_model["mass_vector"], np.float64),
            "strength": 1,
        }
...
def extract_data_from_rng_file(file_name: str, template: dict, entry_id: int) -> dict:
    ...
    assert (
        len(rangefile.rng["molecular_ions"]) <= np.iinfo(np.uint8).max + 1
...
def extract_data_from_rrng_file(file_name: str, template: dict, entry_id) -> dict:
    ...
    # ion indices are on the interval [0, 256)
    assert (
        len(rangefile.rrng["molecular_ions"]) <= np.iinfo(np.uint8).max + 1
    ), "Current implementation does not support more than 256 ion types"
...
def extract_data_from_fig_txt_file(file_name: str, template: dict, entry_id) -> dict:
    ...
    # ion indices are on the interval [0, 256)
    assert (
        len(rangefile.fig["molecular_ions"]) <= np.iinfo(np.uint8).max + 1
    ), "Current implementation does not support more than 256 ion types"
        ...
        if self.meta["file_name"] != "" and self.meta["file_format"] != "none":
            if self.meta["file_format"] == "rng":
                extract_data_from_rng_file(
                    self.meta["file_name"], template, self.meta["entry_id"]
                )
