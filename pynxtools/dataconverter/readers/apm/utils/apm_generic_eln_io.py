...
class NxApmNomadOasisElnSchemaParser:  # pylint: disable=R0903
    ...
    def __init__(self, file_name: str, entry_id: int):
        assert "eln_data" in file_name, "Argument file_name should be eln_data* YAML !"
    ...
    def parse_entry(self, template: dict) -> dict:
        ...
        required_field_names = ["experiment_identifier", "run_number", "operation_mode"]
    ...
    def parse_specimen(self, template: dict) -> dict:
        ...
        for symbol in self.yml[src + ":atom_types"]:
            ...
            assert (symbol in chemical_symbols) and (
                symbol != "X"
            ), "Facing an atom_types list entry which is not an element!"
        template[trg + "atom_types"] = ", ".join(list(self.yml[src + ":atom_types"]))
    ...
    def parse_instrument_header(self, template: dict) -> dict:
        ...
        assert "instrument_name" in self.yml[src].keys(), "instrument_name " + error_msg
        ...
        fpl_exists = ("flight_path_length:value" in self.yml[src].keys()) and (
            "flight_path_length:unit" in self.yml[src].keys()
        )
        assert fpl_exists is True, "flight_path_length " + error_msg
        template[trg + "flight_path_length"] = np.float64(
            self.yml[src + ":" + "flight_path_length:value"]
        )
        template[trg + "flight_path_length/@units"] = self.yml[
            src + ":" + "flight_path_length:unit"
        ]
        fov_exists = ("field_of_view:value" in self.yml[src].keys()) and (
            "field_of_view:unit" in self.yml[src].keys()
        )
        if fov_exists is True:
            template[trg + "field_of_view"] = np.float64(
                self.yml[src + ":" + "field_of_view:value"]
            )
            template[trg + "field_of_view/@units"] = self.yml[
                src + ":" + "field_of_view:unit"
            ]
    ...
    def parse_fabrication(self, template: dict) -> dict:
        ...
        for field_name in required_field_names:
            ...
            template[trg + field_name.replace("fabrication_", "")] = self.yml[
                src + ":" + field_name
            ]
    ...
    def parse_analysis_chamber(self, template: dict) -> dict:
        ...
        for field_name in required_value_fields:
            field_exists = (field_name + ":value" in self.yml[src].keys()) and (
                field_name + ":unit" in self.yml[src].keys()
            )
            assert field_exists is True, field_name + error_msg
            template[trg + field_name.replace("analysis_chamber_", "")] = np.float64(
                self.yml[src + ":" + field_name + ":value"]
            )
            template[
                trg + field_name.replace("analysis_chamber_", "") + "/@units"
    ...
    def parse_reflectron(self, template: dict) -> dict:
        ...
        for field_name in required_field_names:
            assert field_name in self.yml[src].keys(), field_name + error_msg
            template[trg + field_name.replace("reflectron_", "")] = self.yml[
                src + ":" + field_name
            ]
    ...
    def parse_local_electrode(self, template: dict) -> dict:
        ...
        for field_name in required_field_names:
            assert field_name in self.yml[src].keys(), field_name + error_msg
            template[trg + field_name.replace("local_electrode_", "")] = self.yml[
                src + ":" + field_name
            ]
    ...
    def parse_detector(self, template: dict) -> dict:
        ...
        for field_name in required_field_names:
            assert field_name in self.yml[src].keys(), field_name + error_msg
            template[trg + field_name.replace("ion_detector_", "")] = self.yml[
                src + ":" + field_name
            ]
        return template
    ...
    def parse_stage_lab(self, template: dict) -> dict:
        ...
        for field_name in required_value_fields:
            ...
            assert field_exists is True, field_name + error_msg
            template[trg + field_name.replace("stage_lab_", "")] = np.float64(
                self.yml[src + ":" + field_name + ":value"]
            )
            template[trg + field_name.replace("stage_lab_", "") + "/@units"] = self.yml[
                src + ":" + field_name + ":unit"
            ]
    ...
    def parse_specimen_monitoring(self, template: dict) -> dict:
        ...
        error_msg = " is a required field but not found in ELN input!"
        any_required_fields = ["specimen_monitoring_detection_rate"]
        ...
        if section_exists is True:
            ...
            template[trg + "detection_rate"] = np.float64(
                self.yml[src + ":" + any_required_fields[0]]
            )
            ...
            for field_name in any_required_value_fields:
                ...
                template[
                    trg + field_name.replace("specimen_monitoring_", "") + "/@units"
                ] = self.yml[src + ":" + field_name + ":unit"]
    ...
    def parse_control_software(self, template: dict) -> dict:
        ...
        template[trg + "program"] = self.yml[src + ":control_software_program"]
        assert "control_software_program__attr_version" in self.yml[src].keys(), (
            "control_software_program__attr_version " + error_msg
        )
        template[trg + "program/@version"] = self.yml[
            src + ":control_software_program__attr_version"
        ]
    ...
    def parse_pulser(self, template: dict) -> dict:
        ...
        assert "pulse_mode" in self.yml[src].keys(), "pulse_mode" + error_msg
        pulse_mode = self.yml[src + ":" + "pulse_mode"]
        template[trg + "pulse_mode"] = pulse_mode
        assert "pulse_fraction" in self.yml[src].keys(), "pulse_fraction" + error_msg
        template[trg + "pulse_fraction"] = self.yml[src + ":" + "pulse_fraction"]
        template[trg + "pulse_fraction/@units"] = ""
        field_exists = ("pulse_frequency:value" in self.yml[src].keys()) and (
            "pulse_frequency:unit" in self.yml[src].keys()
        )
        assert field_exists is True, "pulse_frequency" + error_msg
        template[trg + "pulse_frequency"] = np.float64(
            self.yml[src + ":pulse_frequency:value"]
        )
        template[trg + "pulse_frequency/@units"] = self.yml[
            src + ":pulse_frequency:unit"
        ]
        ...
        if pulse_mode != "voltage":
            ...
            required_field_names = ["laser_gun_wavelength", "laser_gun_pulse_energy"]
            for field_name in required_field_names:
                field_exists = (field_name + ":value" in self.yml[src].keys()) and (
                    field_name + ":unit" in self.yml[src].keys()
                )
                assert field_exists is True, field_name + error_msg
                template[trg + field_name.replace("laser_gun_", "")] = np.float64(
                    self.yml[src + ":" + field_name + ":value"]
                )
                template[
    ...
    def parse_reconstruction(self, template: dict) -> dict:
        ...
        error_msg = " is a required field but not found in ELN input!"
        assert "program" in self.yml[src].keys(), "program" + error_msg
        ...
        template[trg + "program/@version"] = self.yml[src + ":program__attr_version"]
        required_field_names = [
            "protocol_name",
            "parameter",
