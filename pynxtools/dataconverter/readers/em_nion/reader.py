...
from pynxtools.dataconverter.readers.em_nion.utils.em_io_utils_misc import (
    recursive_query_nested_dict,
    assess_situation_with_input_files,
    NION_ERROR_CODES,
)

from pynxtools.dataconverter.readers.em_nion.utils.em_io_utils_ods2dct import (
    ods_to_json_routing_dict,
)
...
def extract_data_from_json_file(file_name: str, template: dict) -> dict:
    ...
    for keyword, value in json_to_nxdl_path.items():
        ...
        assert len(value) == 2, "Keyword: " + keyword + "\n" + "Value is not a duplett!"
        ...
        if nxdl_path in template.keys():
            ...
            assert tmp is not None, "Keyword: " + keyword + "\n" + "Value is None!"
    ...
    template["/ENTRY[entry]/start_time"] = value = (
        date_time + time_zone[0:3] + ":" + time_zone[3:5]
    )
    # NEW ISSUE:
    template["/ENTRY[entry]/end_time"] = template["/ENTRY[entry]/start_time"]
    template["/ENTRY[entry]/SAMPLE[sample]/preparation_date"] = template[
    ...
    assert center_positions != [], "Value for center positions is an empty list!"
    assert len(center_positions) == 2, "Value for center positions is not a duplett!"

    template["/ENTRY[entry]/em_lab/hadf/SCANBOX_EM[scanbox_em]/center"] = np.asarray(
        center_positions, np.float64
    )
    template["/ENTRY[entry]/em_lab/hadf/SCANBOX_EM[scanbox_em]/center/@units"] = "nm"
    ...
    for axis in [0, 1, 2]:
        ...
        positions[axis] = tmp
    template["/ENTRY[entry]/em_lab/STAGE_LAB[stage_lab]/position"] = positions
    ...
    template["/ENTRY[entry]/em_lab/miscellaneous/semi_convergence_angle"] = tmp
    template["/ENTRY[entry]/em_lab/miscellaneous/semi_convergence_angle/@units"] = "rad"

    return template


def extract_data_from_scans(
    npy_file_name: str, json_file_name: str, template: dict
) -> dict:
    ...
    assert isinstance(scan_sizes, list), "Value for scan sizes is not a list!"
    assert scan_sizes != [], "Value for scan sizes is an empty list!"
    assert len(scan_sizes) == 2, "Value for scan sizes is not a duplett!"

    xmin = (
        center_positions[0]
        + spatial_calibrations[0]["offset"]
...
def extract_data_from_elabftw_file(file_name: str, template: dict) -> dict:
    ...
    for nxdl_path, value in dct.items():
        if nxdl_path in template.keys():
            assert value is not None, "Keyword: " + nxdl_path + "\n" + "Value is None!"
...
class EmNionReader(BaseReader):
    ...
    def read(
        ...
        if "json" in typed_files:
            assert len(typed_files["json"]) == 1, "List of json files is ambiguous!"
        ...
        if "npy" in typed_files:
            assert len(typed_files["npy"]) == 1, "List of npy files is ambiguous!"
            assert len(typed_files["json"]) == 1, "List of json files is ambiguous!"

            extract_data_from_scans(
                typed_files["npy"][0], typed_files["json"][0], template
            )
