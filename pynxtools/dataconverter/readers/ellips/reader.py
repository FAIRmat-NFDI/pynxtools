...
class EllipsometryReader(BaseReader):
    ...
    def populate_header_dict_with_datasets(file_paths):
        ...
        my_numpy_array = np.empty(
            [1, 1, len(unique_angles), len(["psi", "delta"]), counts[0]]
        )

        for index, unique_angle in enumerate(unique_angles):
            my_numpy_array[0, 0, index, :, :] = unique_angle

        for index in range(len(labels["psi"])):
            my_numpy_array[0, 0, index, 0, :] = (
                whole_data["psi"]
    ...
    def read(
        ...
        template["/ENTRY[entry]/plot/wavelength"] = {
            "link": "/entry/instrument/spectrometer/wavelength"
        }
        template["/ENTRY[entry]/plot/wavelength/@units"] = "angstrom"

        for index, psi in enumerate(psilist):
            template[f"/ENTRY[entry]/plot/{psi}"] = {
                "link": "/entry/sample/measured_data",
                "shape": np.index_exp[0, 0, index, 0, :],
            }
            template[f"/ENTRY[entry]/plot/{psi}/@units"] = "degrees"

        for index, delta in enumerate(deltalist):
            template[f"/ENTRY[entry]/plot/{delta}"] = {
                "link": "/entry/sample/measured_data",
                "shape": np.index_exp[0, 0, index, 1, :],
            }
