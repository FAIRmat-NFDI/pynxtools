...
from pynxtools.nyaml2nxdl.nyaml2nxdl_helper import (
    ...
    def print_root_level_doc(self, file_out):
        ...
        file_out.write(
            "{indent}{root_level_doc}\n".format(
                indent=0 * DEPTH_SIZE, root_level_doc=self.root_level_doc
            )
        )
        self.root_level_doc = ""
        ...
        if self.root_level_symbols:
            file_out.write(
                "{indent}{root_level_symbols}\n".format(
                    indent=0 * DEPTH_SIZE, root_level_symbols=self.root_level_symbols
                )
            )
            for symbol in self.symbol_list:
                file_out.write(
                    "{indent}{symbol}\n".format(indent=0 * DEPTH_SIZE, symbol=symbol)
                )
        if self.root_level_definition:
            ...
            for defs in self.root_level_definition:
                ...
                if defs in ("category: application", "category: base"):
                    continue
                file_out.write(
                    "{indent}{defs}\n".format(indent=0 * DEPTH_SIZE, defs=defs)
                )
            file_out.write(
                "{indent}{defs}\n".format(indent=0 * DEPTH_SIZE, defs=nx_name)
            )
        self.found_definition = False
        ...
        if check_doc:
            ...
            for child in list(node):
                ...
                if tag == ("item"):
                    file_out.write(
                        "{indent}{value}: \n".format(
                            indent=(depth + 1) * DEPTH_SIZE, value=child.attrib["value"]
                        )
                    )
                    if list(child):
                        for item_doc in list(child):
                            ...
                            self.handle_not_root_level_doc(
                                item_doc_depth, item_doc.text, item_doc.tag, file_out
                            )
        else:
            file_out.write(
                "{indent}{tag}:".format(
            ...
            file_out.write(" [{enum_list}]\n".format(enum_list=enum_list[:-2] or ""))
