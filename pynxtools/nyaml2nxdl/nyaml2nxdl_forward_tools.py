...
def xml_handle_dimensions(dct, obj, keyword, value: dict):
    ...
    assert "dim" in value.keys(), (
        f"Line {dct[line_number]}: No dim as child of dimension has " f"been found."
    )
...
def xml_handle_enumeration(dct, obj, keyword, value, verbose):
    ...
    assert (
        value is not None
    ), f"Line {dct[line_number]}: enumeration must \
...
def xml_handle_symbols(dct, obj, keyword, value: dict):
    ...
    ), f"Line {dct[line_number]}: symbols table must not be empty !"
    ...
    for kkeyword, vvalue in value.items():
        ...
        if kkeyword != "doc":
            ...
            assert vvalue is not None and isinstance(
                vvalue, str
            ), f"Line {value[line_number]}: put a comment in doc string !"
