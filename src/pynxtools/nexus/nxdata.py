...
# ---------------------------------------------------------------------------
# Helpers kept for backward-compat with annotator/nxdata.py callers
# ---------------------------------------------------------------------------
...
def signal_helper(nxdata: h5py.Group) -> h5py.Dataset | None:
    ...
    for key in nxdata.keys():
        if isinstance(nxdata[key], h5py.Dataset):
            datasets.append(nxdata[key])
    if len(datasets) == 1:
        return datasets[0]
    # v2: select the one with @signal="1"
    for ds in datasets:
        signal_attr = decode_if_string(ds.attrs.get("signal"))
        if signal_attr and signal_attr == "1":
            return ds
    return None
