...
NOMAD_PACKAGE_DIR = Path(__file__).resolve().parent
...
def resolve_artifact_path(
    *,
    ...
    """Resolve the dir for a generated artifact.

    Resolution order:

    1. If ``build_env_var`` is ``"1"``, always return ``package_dir / filename``.
    2. If the file exists in ``package_dir``, return that path.
    3. Otherwise, return ``cache_dir / filename``.
