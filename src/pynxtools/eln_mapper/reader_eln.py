...
__all__ = ["ReaderElnGenerator"]

from pynxtools.eln_mapper.eln import ElnGenerator
from pynxtools.nexus.nexus_tree import NexusField, NexusGroup, _NexusEntityBase
...
class ReaderElnGenerator(ElnGenerator):
    ...
    def _construct_entity_structure(
        ...
        field_unit = node.unit if isinstance(node, NexusField) else None

        if not node.children and not field_unit:
