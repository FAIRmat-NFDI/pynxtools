...
def get_prev_element(x, instrument, str_replacement = "/entry/instrument/"):
  ...
  prev_element_output = _get_value(instrument[shorten_path + '/previous_devices'])
...
def get_pos_vector_from_opt_element(x, instrument, str_replacement = "/entry/instrument/"):
  ...
  length = _get_value(instrument[shorten_path + '/location/coordinates'])
  vector = instrument[shorten_path + '/location/coordinates'].attrs['vector']
...
def plot_all_pahts(dictionary):
  ...
  for i, ax in enumerate(axes):
      ...
      ax.plot(beam_names, atten, label='Attentuation')
      ax.plot(beam_names, div, label='Divergence')


      # Customize subplot
      ax.set_title(f'Subplot {selected_path}')
      ax.legend()
      ax.set_ylabel("Attenuation / Divergence")
      ax.set_xlabel("Beam name")
      ax.legend()
      ax.grid()
...
def add_position_and_names_to_graph_nodes(Graph,instrument):
  ...
  for item in Graph.nodes:
    ...
    print(item, "##", item.replace('/entry/instrument/',''))
...
if __name__ == "__main__":
  ...
  NeXus_File_Name = "NXraman_example_v3.nxs"
  #NeXus_File_Name = "Beam Path Modeling v4/interferometer_v4.nxs"
  ...
  Target_Path = '/entry/instrument/'
  Groups, Fields = get_groups_and_datasets_from_nexus_file(NeXus_File_Name, Target_Path)
  
  #A = 2#filter this set to only get the datasets with attributes NX_class = NXbeam_path



  Groups_with_NXbeam_device=filter_group_for_NeXus_class(Groups,NeXus_File_Name,Target_Path, "NXbeam_device")
  #C=check_if_graph_edge_has_attribute(A[1], "NXbeam_device")


  if False:
    # Vs Temp, Vs Pressure, Vs Time delay, Vs Pulse length,
    # Number of Incident Beams, 
  ...
  prefix = '/entry/instrument/'
  Groups_with_NXbeam_device_with_prefix =[]

  for i in range(len(Groups_with_NXbeam_device)):
    Groups_with_NXbeam_device_with_prefix.append(prefix +  Groups_with_NXbeam_device[i])

  node_list = Groups_with_NXbeam_device_with_prefix
  ...
  # Use this to plot the setup without the beam names
  if True:

    node_labels = nx.get_node_attributes(G, 'name') 
    nx.draw(G, pos=node_positions,  with_labels=True, labels=node_labels,node_size=700, node_color="skyblue", font_size=10, font_color="black", font_weight="bold", edge_color="gray", linewidths=1, alpha=0.7)
  ...
  if False:
    ...
    #source_filename = 'Beam Path Modeling v4/interferometer_v4.nxs'


    # Destination file name (new name for the copy)
    destination_filename = 'interferometer_v4_mod.nxs'
    #destination_filename = 'Beam Path Modeling v4/interferometer_v4_mod.nxs'
    ...
    #f = h5py.File("Beam Path Modeling v4/interferometer_v4_mod.nxs", "a")
